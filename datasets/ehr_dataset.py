...
def get_datasets(discretizer, normalizer, args):
    ...
    transform = None
    # print("reading ehr data from /tmpdata")
    ...
    # train_ds = EHRdataset(args, discretizer, normalizer, f'{args.tmp_dir}/{args.task}/train_listfile.csv', os.path.join(args.tmp_dir, f'{args.task}/train'), transforms=transform)
    # val_ds = EHRdataset(args, discretizer, normalizer, f'{args.tmp_dir}/{args.task}/val_listfile.csv', os.path.join(args.tmp_dir, f'{args.task}/train'), transforms = transform)
    # test_ds = EHRdataset(args, discretizer, normalizer, f'{args.tmp_dir}/{args.task}/test_listfile.csv', os.path.join(args.tmp_dir, f'{args.task}/test'), transforms = transform)
...
def get_data_loader(discretizer, normalizer, dataset_dir, batch_size):
    ...
    train_dl = DataLoader(train_ds, batch_size, shuffle=False, collate_fn=my_collate, pin_memory=True, num_workers=16)
