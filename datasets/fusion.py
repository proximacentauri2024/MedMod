...
def loadmetadata(args):
    ...
    if (args.dataset!='all'):
        ...
        if args.task == 'phenotyping' and args.labels_set =='pheno' or args.task == 'readmission':
            end_time = cxr_merged_icustays.outtime
            cxr_merged_icustays_during = cxr_merged_icustays.loc[(cxr_merged_icustays.StudyDateTime>=cxr_merged_icustays.intime)&((cxr_merged_icustays.StudyDateTime<=end_time))]
        # need to change this 
        if args.task == 'phenotyping' and args.labels_set == 'radiology':
            end_time = cxr_merged_icustays.outtime
            cxr_merged_icustays_during = cxr_merged_icustays.loc[(cxr_merged_icustays.StudyDateTime>=cxr_merged_icustays.intime)&((cxr_merged_icustays.StudyDateTime<=end_time))]
...
def load_cxr_ehr(args, ehr_train_ds, ehr_val_ds, cxr_train_ds, cxr_val_ds, ehr_test_ds, cxr_test_ds):
    ...
    print(f"Number of positive train examples", total_labels_count )
    # print("pos", train_ds.ehr_ds.data_map['labels'].value_count())
    ...
    for key, value in val_ds.ehr_ds.data_map.items():
        ...
        for label in labels:
            if label == 1:  # Assuming 1 indicates a positive label
                total_labels_count += 1
    print(f"Number of positive val examples", total_labels_count )
    ...
    for key, value in test_ds.ehr_ds.data_map.items():
        ...
        for label in labels:
            if label == 1:  # Assuming 1 indicates a positive label
                total_labels_count += 1
    print(f"Number of positive test examples", total_labels_count )
